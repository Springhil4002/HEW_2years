...
void Goal::Init()
{
	SetTex("asset/Texture/Door.png");					// âÊëúì«Ç›çûÇ›
	SetScale(BLOCK_SIZE, BLOCK_SIZE * 2, 0.0f);			// ëÂÇ´Ç≥Çê›íË
}
...
void Goal::Update()
{
	...
	for (auto& pl : player)
	{
		if (Object::Collision(this, pl))
		{
			...
			{
				SceneManager::m_SoundManager.Play(SOUND_LABEL_SE010);	// ÉSÅ[Éãâπ
